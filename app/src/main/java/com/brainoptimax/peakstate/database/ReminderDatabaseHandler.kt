...
class ReminderDatabaseHandler(context: Context) :
    SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    companion object {
        private const val DATABASE_VERSION = 1
        private const val DATABASE_NAME = "ReminderPS"
    ...
    fun saveReminder(reminder: Reminders): Long {
        val db = this.writableDatabase
        val contentValues = ContentValues()
        contentValues.put(TITLE, reminder.title)
        contentValues.put(SUBTITLE, reminder.subtitle)
    ...
    fun getReminderById(reminderId: Long): Reminders {
    ...
    fun updateReminder(reminder: Reminders): Int {
        val db = this.writableDatabase
        val contentValues = ContentValues()
    ...
    fun getAll(): MutableList<Reminders> {
        ...
        if (cursor.moveToFirst()) {
            while (!cursor.isAfterLast) {
                ...
                val createdTime = cursor.getLong(6)
                val modifiedTime = cursor.getLong(7)
